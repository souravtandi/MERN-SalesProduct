...
const authMiddleware = require('../middleware/protected_routes');

const SalesModel = mongoose.model("SalesModel");

router.post('/store-product-sale', authMiddleware, (req, res)=>{
...
router.get("/top-five-sales", authMiddleware, (req, res)=>{
    SalesModel.find()
    .sort({quantity: -1})
    .limit(5)
    .exec(
